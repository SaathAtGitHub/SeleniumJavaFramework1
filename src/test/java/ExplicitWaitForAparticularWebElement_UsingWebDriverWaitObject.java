...
public class ExplicitWaitForAparticularWebElement_UsingWebDriverWaitObject {
	...
	public static void main(String[] args) 
	{
		...
		// U can create explicit wait using below two statements. Explicit wait is dedicated to a particular WebElement while implicitly wait is applicable to the whole browser (session) 
		WebDriverWait wait = new WebDriverWait(driver, 20);
		WebElement element = wait.until(ExpectedConditions.elementToBeClickable(By.name("abcdef"))); 
		// In above statement, there's no element with "abcdef" name but since we want to check if the WebDriver is going to wait here for 20 seconds before 
		// it throws "No Such Element" exception error. Explicit wait can be set for specific elements, also Explicit wait can be set for a specific condition.
